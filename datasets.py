...
class ImageDataset_sRGB_from_folder(Dataset):
	def __init__(self, root, mode="train", verbose = True,
			# unpaird_data="fiveK", combined=True
		):
		self.mode = mode
		...
		assert os.path.isdir(train_dir), f"train dir (train/) not found in {root}"
		assert os.path.isdir(test_dir), f"test dir (test/) not found in {root}"
		if verbose:
			print(f'loading dataset from folder {train_dir} and {test_dir}')

		# loading training set
		input_dir = os.path.join(train_dir, 'input')
		output_dir = os.path.join(train_dir, 'output')
		assert os.path.isdir(input_dir), f"input dir (input/) not found in {train_dir}"
		assert os.path.isdir(output_dir), f"output dir (output/) not found in {train_dir}"
		file_list = [ f for f in os.listdir(input_dir) if os.path.isfile(os.path.join(output_dir, f))]
		file_list = [ f for f in file_list
					if Image.open(os.path.join(input_dir, f)).size == Image.open(os.path.join(output_dir,f)).size
					]
		self.set1_input_files = [os.path.join(input_dir, f) for f in file_list]
		self.set1_expert_files = [os.path.join(output_dir, f) for f in file_list]
		if verbose:
			print(f'Found {len(self.set1_input_files)} training images.')

		# loading test set
		input_dir = os.path.join(test_dir, 'input')
		output_dir = os.path.join(test_dir, 'output')
		assert os.path.isdir(input_dir), f"input dir (input/) not found in {test_dir}"
		assert os.path.isdir(output_dir), f"output dir (output/) not found in {test_dir}"
		file_list = [ f for f in os.listdir(input_dir) if os.path.isfile(os.path.join(output_dir, f))]
		file_list = [ f for f in file_list
					if Image.open(os.path.join(input_dir, f)).size == Image.open(os.path.join(output_dir,f)).size
					]
		self.test_input_files = [os.path.join(input_dir, f) for f in file_list]
		self.test_expert_files = [os.path.join(output_dir, f) for f in file_list]
		if verbose:
			print(f'Found {len(self.test_input_files)} testing images.')
