...
parser.add_argument('--dataset_dir', type = str, required = True, help = "Path to Dataset directory containing sub-directories of train and test of which each also contains sub-direcotries of input and output")
...
if opt.input_color_space == 'sRGB':
	dataloader = DataLoader(
		...
		ImageDataset_sRGB_from_folder(opt.dataset_dir, mode = "train"),
	...
	psnr_dataloader = DataLoader(
		...
		ImageDataset_sRGB_from_folder(opt.dataset_dir, mode = "test"),
		batch_size=1,
		shuffle=False,
		num_workers=1,
	)
